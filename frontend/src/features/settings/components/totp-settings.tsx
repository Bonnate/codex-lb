      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center justify-between rounded-lg border p-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">비밀번호 로그인 후 TOTP 코드를 추가로 요구합니다.</p>
          </div>
          <Switch
      ...
      <Dialog open={activeDialog === "setup"} onOpenChange={(open) => !open && closeDialog()}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              인증 앱으로 QR 코드를 스캔한 뒤 인증 코드를 입력하세요.
            </DialogDescription>
          </DialogHeader>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-col items-center gap-2">
                      <FormLabel className="sr-only">인증 코드</FormLabel>
                      <FormControl>
                        <InputOTP
                          maxLength={6}
                          value={field.value}
      ...
      <Dialog open={activeDialog === "disable"} onOpenChange={(open) => !open && closeDialog()}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>2단계 인증을 끄려면 현재 TOTP 코드를 입력하세요.</DialogDescription>
          </DialogHeader>
          {error ? <AlertMessage variant="error">{error}</AlertMessage> : null}
          <Form {...disableForm}>
