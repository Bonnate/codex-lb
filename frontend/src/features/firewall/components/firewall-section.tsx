...
function modeLabel(mode: "allow_all" | "allowlist_active"): string {
  return mode === "allow_all" ? "모두 허용" : "허용 목록 활성";
}
    ...
    <section className="space-y-3 rounded-xl border bg-card p-5">
      <div className="flex items-center gap-2.5">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">허용된 클라이언트 IP만 프록시 API를 사용할 수 있게 제한합니다.</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 rounded-lg border px-3 py-2">
        ...
        <div className="flex items-center gap-1.5">
          <span className="text-xs text-muted-foreground">허용 IP</span>
      ...
      <div className="flex flex-col gap-2 sm:flex-row">
        <Input
          ...
          placeholder="127.0.0.1 또는 2001:db8::1"
      ...
      <ConfirmDialog
        ...
        title="허용 목록에서 IP 제거"
        description={`${deleteDialog.data ?? ""} IP는 더 이상 방화벽을 통과할 수 없습니다.`}
        confirmLabel="제거"
