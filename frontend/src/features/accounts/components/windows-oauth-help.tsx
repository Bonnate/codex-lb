...
export function WindowsOauthHelp() {
  ...
  return (
    <div className="space-y-3 rounded-xl border bg-card p-4">
      <div>
        <h2 className="text-sm font-semibold">Windows OAuth 안내</h2>
        <p className="mt-1 text-xs text-muted-foreground">
          Windows에서 브라우저 로그인이 실패하면 로컬 포트 포워딩을 만들어 OAuth 콜백이
          <code className="mx-1 rounded bg-muted px-1 py-0.5">localhost:1455</code>
          에서 이 서버로 도달하도록 설정하세요.
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="text-xs text-muted-foreground">PowerShell을 관리자 권한으로 실행한 뒤 사용하세요.</span>
      </div>
    </div>
  );
}
