...
function getRoutingLabel(strategy: "usage_weighted" | "round_robin" | "capacity_weighted", sticky: boolean, preferEarlier: boolean): string {
  if (strategy === "round_robin") {
    return sticky ? "라운드 로빈 + 스티키 스레드" : "라운드 로빈";
  }
  if (strategy === "capacity_weighted") {
    if (sticky && preferEarlier) return "용량 가중치 + 스티키 + 빠른 리셋 우선";
    if (sticky) return "용량 가중치 + 스티키 스레드";
    if (preferEarlier) return "용량 가중치 + 빠른 리셋 우선";
    return "용량 가중치";
  }
  if (sticky && preferEarlier) return "스티키 + 빠른 리셋 우선";
  if (sticky) return "스티키 스레드";
